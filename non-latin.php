<?php
/*
Plugin Name: non-latin attachments
Description: This plugin change filename to numbers. And when the file is downloaded, the attachment's title will be set as filename.
 ...
 * 파일을 첨부할 때 원본 파일명으로 첨부 파일의 post_title 을 넣어 준다.
...
function non_latin_attachments_add_attachment($attachment_id)
{
    ...
    if ($non_latin_attachments_need_change == 'yes') {
        ...
        $attachment->post_title = str_replace( '.' . pathinfo($original_filename, PATHINFO_EXTENSION), '', $original_filename);

        //첨부파일 타이틀 업데이트에 실패하면 에러 메시지를 출력하고 죽는다.
        if (wp_update_post($attachment) === 0) {
            echo 'error occured!';
...
function non_latin_attachments_get_download_url()
{
    ...
    foreach ($_REQUEST['attachments'] as $guid) {
        ...
        if (!strstr($filetype['type'], 'image')) {
            ...
            // If there is post.
            if ($post) {
            	$title_extension = pathinfo($post->post_title, PATHINFO_EXTENSION);
            	$file_extension = pathinfo($guid, PATHINFO_EXTENSION);
